...
function createRequest(method, url, { proxy, ssl }) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (ssl) {
            ...
            if (cafiles.length > 0) {
                req.ca(yield Promise.all(cafiles.map(p => fs_1.fsReadFile(p, { encoding: 'utf8' }))));
            }
            if (certfiles.length > 0) {
                req.cert(yield Promise.all(certfiles.map(p => fs_1.fsReadFile(p, { encoding: 'utf8' }))));
            }
            if (keyfiles.length > 0) {
                req.key(yield Promise.all(keyfiles.map(p => fs_1.fsReadFile(p, { encoding: 'utf8' }))));
            }
        }
        return { req };
    });
